...
datasource0 = glueContext.create_dynamic_frame.from_catalog(database = "de_youtube_crawl_database", table_name = "raw_statistics", transformation_ctx = "datasource0", push_down_predicate = predicate_pushdown)
...
datasink4 = glueContext.write_dynamic_frame.from_options(frame = df_final_output, connection_type = "s3", connection_options = {"path": "s3://de-youtube-cleansed-csv-parquet/youtube/raw_statiistics/", "partitionKeys": ["region"]}, format = "parquet", transformation_ctx = "datasink4")

# /AmazonS3_node1726902480948 = glueContext.write_dynamic_frame.from_options(frame=AWSGlueDataCatalog_node1726902320173, connection_type="s3", format="glueparquet", connection_options={"path": "s3://de-youtube-cleansed-csv-parquet/youtube/raw_statiistics/", "partitionKeys": []}, format_options={"compression": "snappy"}, transformation_ctx="AmazonS3_node1726902480948")
